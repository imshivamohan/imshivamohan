...
if ($dockerRunning -and $rancherDesktopRunning) {
    ...
    if (-Not (Test-Path $keystoreFile) -or -Not (Test-Path $truststoreFile)) {
        ...
        # Prompt user for the keystore and truststore files and passwords
        ...
        $keystorePassword = Read-Host "Please enter the keystore password" -AsSecureString
        $truststorePassword = Read-Host "Please enter the truststore password" -AsSecureString
